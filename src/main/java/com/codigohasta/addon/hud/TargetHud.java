...
public class TargetHud extends HudElement {
    ...
    private void renderTH(HudRenderer r, double px, double py, double bw, double bh, float s) {
        ...
        if (showHealth.get()) {
            ...
            animHealth = Mth.lerp(0.15, animHealth, target.getHealth());
            double pct = Mth.clamp(animHealth / target.getMaxHealth(), 0, 1);
            if (pct > 0.01) {
        ...
        if (showArmor.get() && target instanceof Player pe)
            drawArmor(r, textX, barY - 16 * s, s, pe);

        if (showPotions.get() && target instanceof Player pe)
    ...
    private void drawSkin(HudRenderer r, double x, double y, double size, double size2, double rad, Color bgColor) {
        if (target instanceof AbstractClientPlayer ace) {
            ...
            if (skinID != null) {
                ...
                Renderer2D.TEXTURE.render(tex.getTextureView(), tex.getSampler());
            }
        } else {
    ...
    private Identifier getSkinTexture(AbstractClientPlayer player) {
        try {
            PlayerInfo entry = ((AbstractClientPlayerEntityAccessor) player).invokeGetPlayerListEntry();
            if (entry != null) {
                return entry.getSkin().body().texturePath();
            }
        } catch (Exception ignored) {}
        return DefaultPlayerSkin.get(player.getUUID()).body().texturePath();
    }

    private void drawArmor(HudRenderer r, double x, double y, float s, Player pe) {
        ItemStack[] items = {
            pe.getMainHandItem(),
            pe.getItemBySlot(net.minecraft.world.entity.EquipmentSlot.HEAD),
            pe.getItemBySlot(net.minecraft.world.entity.EquipmentSlot.CHEST),
            pe.getItemBySlot(net.minecraft.world.entity.EquipmentSlot.LEGS),
            pe.getItemBySlot(net.minecraft.world.entity.EquipmentSlot.FEET),
            pe.getOffhandItem()
        };
        int ix = (int) x;
    ...
    private void drawArmorSmall(HudRenderer r, double x, double y, float s, Player pe) {
        ItemStack[] items = {
            pe.getMainHandItem(),
            pe.getItemBySlot(net.minecraft.world.entity.EquipmentSlot.HEAD),
            pe.getItemBySlot(net.minecraft.world.entity.EquipmentSlot.CHEST),
    ...
    private void drawPotionText(HudRenderer r, double x, double y, Player pe) {
        StringBuilder sb = new StringBuilder();
        for (MobEffectInstance effect : pe.getActiveEffects()) {
            var type = effect.getEffect().value();
            if (type == MobEffects.STRENGTH) sb.append("Str").append(effect.getAmplifier()+1).append(" ");
            else if (type == MobEffects.SPEED) sb.append("Spd").append(effect.getAmplifier()+1).append(" ");
            else if (type == MobEffects.REGENERATION) sb.append("Reg ");
            else if (type == MobEffects.RESISTANCE) sb.append("Res").append(effect.getAmplifier()+1).append(" ");
            else if (type == MobEffects.WEAKNESS) sb.append("Weak ");
            else if (type == MobEffects.SLOWNESS) sb.append("Slow").append(effect.getAmplifier()+1).append(" ");
        }
        if (!sb.isEmpty())
    ...
    private void updateTarget() {
        ...
        if (mc.hitResult instanceof EntityHitResult hit && hit.getEntity() instanceof LivingEntity le) {
            target = le; return;
        }
        target = null;
    }
