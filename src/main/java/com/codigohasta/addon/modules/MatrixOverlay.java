...
public class MatrixOverlay extends Module {
    private final SettingGroup sgGeneral = settings.getDefaultGroup();

    private final Setting<Double> flowSpeed = sgGeneral.add(new DoubleSetting.Builder().name("flow-speed").defaultValue(3.0).min(0).sliderMax(10).build());
    ...
    private final Setting<Boolean> loopScan = sgGeneral.add(new BoolSetting.Builder().name("loop-scan").defaultValue(true).build());
    private final Setting<Double> scanDuration = sgGeneral.add(new DoubleSetting.Builder().name("scan-duration").defaultValue(4.0).min(1.0).sliderMax(10.0).visible(loopScan::get).build());

    public MatrixOverlay() {
        super(com.codigohasta.addon.AddonTemplate.CATEGORY, "matrix-overlay", "Digital Matrix Radar Shader");
    }
    ...
    private void onRender3D(Render3DEvent event) {
        ...
        GL20.glUniform1f(GL20.glGetUniformLocation(programId, "U_GridSpeed"), flowSpeed.get().floatValue());
        GL20.glUniform1f(GL20.glGetUniformLocation(programId, "U_SunSpeed"), 1.0f);
    ...
    private void loadShader() {
        try {
            int v = createShader("matrix.vsh", GL20.GL_VERTEX_SHADER);
            int f = createShader("matrix.fsh", GL20.GL_FRAGMENT_SHADER);
