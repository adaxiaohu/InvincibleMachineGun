...
public class TpAura extends Module {
    ...
    public TpAura() {
        super(AddonTemplate.CATEGORY, "如来神掌", "如来神掌。从天而降的掌法。");
    }
                ...
                for (int yOffset = 3; yOffset >= -3; yOffset--) { // 稍微缩小垂直搜索范围，提高效率
                    ...
                    if (isSafeStandableBlock(checkPos)) {
                        ...
                        if (mc.world.isSpaceEmpty(spaceCheck)) {
                            // 3. 确保这个位置在玩家的攻击范围内
                            if (mc.player.getPos().distanceTo(safePos) <= maxRange.get()) {
                                return safePos; // 找到了！
                            }
                        }
                    }
            ...
            for (int z = -2; z <= 2; z++) {
                for (int y = -1; y <= 1; y++) { // 在目标垂直方向上小范围搜索
                    ...
                    if (isSafeFluidBlock(mc.world, checkPos)) {
                        ...
                        if (mc.player.getPos().distanceTo(fluidPos) <= maxRange.get()) {
    ...
    private Vec3d findAirborneAttackPos(Entity target) {
        ...
        if (mc.player.getPos().distanceTo(target.getPos()) <= maxRange.get()) {
