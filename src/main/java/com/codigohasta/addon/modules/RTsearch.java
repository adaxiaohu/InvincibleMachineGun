...
public class RTsearch extends Module {
    ...
    private final Setting<Integer> rtpDelay = sgGeneral.add(new IntSetting.Builder()
        ...
        .defaultValue(3) // 默认值改为3秒
        .min(1)          // 最小允许1秒 (建议不要0，0可能会导致还没加载就判断)
        .max(30)
        .sliderMin(1)
        .sliderMax(10)   // 滑块范围改小，方便你拖动
        .build()
    );
    ...
    public RTsearch() {
        super(AddonTemplate.CATEGORY, "RTsearch", "RTP to specific coordinates or find specific biomes using /rt.");
    }

    @Override
    public void onActivate() {
        // --- 修改点：开启即传送 ---
        tickTimer = rtpDelay.get() * 20; // 让计时器直接满足条件
        ...
        if (rtpMode.get() == RTPMode.COORDINATES) {
            ...
            info("RTsearch started - target: (%d, %d)", targetX.get(), targetZ.get());
            ...
            info("RTsearch started - Biome Finder mode");
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        // --- 修改点：超时保护 ---
        // 硬编码为5秒超时（防止你设置1秒延迟时，还没传送到位就因为超时重置了）
        // 只有超过5秒没反应，才强制重置状态。
        if (isRtping && tickTimer >= 5 * 20) { 
            isRtping = false;
        }

        // 只要满足用户设定的延迟(rtpDelay)，并且当前没有正在进行的RTP(isRtping=false)，就执行
    ...
    private void handleCoordinatesMode() {
        ...
        if (tickTimer % 20 == 0 && Math.abs(currentDistance - lastReportedDistance) > 100) {
            // 降低刷屏频率
        }
    }
    ...
    private void handleBiomeMode() {
        if (biomeFound) {
            ...
            if (disconnectOnReach.get()) {
                info("Disconnecting...");
                disconnectWithMessage("RTsearch: found requested biome");
            }
        ...
        if (event.packet instanceof PlayerPositionLookS2CPacket && mc.player != null) {
            ...
            if (lastRtpPos == null || !currentPos.equals(lastRtpPos)) {
                ...
                if (rtpMode.get() == RTPMode.COORDINATES) {
                    ...
                    info("RTP %d done - dist: %.1f", rtpAttempts, distance);
                    lastReportedDistance = distance;
                } else {
                    String biome = getCurrentBiome();
                    info("RTP %d done - biome: %s", rtpAttempts, biome);
    ...
    private void performRTP() {
        ...
        if (rtpMode.get() == RTPMode.COORDINATES) {
            // double currentDistance = getCurrentDistance();
            // info("Wait RTP... curr: %.1f", currentDistance);
        } else {
            // info("Wait RTP... target: %s", targetBiome.get().getDisplayName());
        }
    }
