...
public class ScaffoldPlus extends Module {
    public ScaffoldPlus() {
        super(AddonTemplate.CATEGORY, "L搭路", "同样是来自leaveshack的自动搭路，早些时候抄的。现在接上了rotation。grim能搭但是反作弊标记一堆。");
    }
    ...
    private void onRender3d(Render3DEvent event) {
        ...
        ItemStack stack = mc.player.getInventory().getStack(((InventoryAccessor)mc.player.getInventory()).getSelectedSlot());
        ...
        if (slabMode){
            if (stack.getItem() instanceof BlockItem blockItem && blockItem.getBlock() instanceof SlabBlock && !BlockUtil.shiftBlocks.contains(Block.getBlockFromItem(stack.getItem())) && ((BlockItem) stack.getItem()).getBlock() != Blocks.COBWEB) {
                block = ((InventoryAccessor)mc.player.getInventory()).getSelectedSlot();
            }
        }else {
