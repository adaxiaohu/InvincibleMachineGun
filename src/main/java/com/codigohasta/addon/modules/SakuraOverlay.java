...
public class SakuraOverlay extends Module {
    private final SettingGroup sgGeneral = settings.getDefaultGroup();

    private final Setting<Double> petalDensity = sgGeneral.add(new DoubleSetting.Builder()
        .name("petal-density")
        .description("Amount of falling cherry blossom petals.")
        .defaultValue(1.0).min(0).sliderMax(5.0).build());

    private final Setting<Double> pinkIntensity = sgGeneral.add(new DoubleSetting.Builder()
        .name("pink-intensity")
        .description("How much pink tint to apply.")
        .defaultValue(0.4).min(0).sliderMax(1.0).build());

    private final Setting<Double> scanSpeed = sgGeneral.add(new DoubleSetting.Builder().name("bloom-speed").defaultValue(1.0).min(0.1).sliderMax(5.0).build());
    private final Setting<Boolean> loopScan = sgGeneral.add(new BoolSetting.Builder().name("loop-bloom").defaultValue(false).build());

    public SakuraOverlay() {
        super(com.codigohasta.addon.AddonTemplate.CATEGORY, "sakura-overlay", "Aesthetic Cherry Blossom Atmosphere Shader");
    }
    ...
    private void onRender3D(Render3DEvent event) {
        ...
        // 参数传递
        GL20.glUniform1f(GL20.glGetUniformLocation(programId, "U_PetalDensity"), petalDensity.get().floatValue());
        GL20.glUniform1f(GL20.glGetUniformLocation(programId, "U_PinkIntensity"), pinkIntensity.get().floatValue());
        ...
        GL20.glUniform1f(GL20.glGetUniformLocation(programId, "U_ScanDuration"), 6.0f);
    ...
    private void loadShader() {
        try {
            int v = createShader("sakura.vsh", GL20.GL_VERTEX_SHADER);
            int f = createShader("sakura.fsh", GL20.GL_FRAGMENT_SHADER);
